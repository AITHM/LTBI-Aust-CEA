...
options(scipen=5)
dev.off()
ggplot(data, aes(x = incremental.qalys/1000, y = incremental.cost/1000000,
                 fill = strategy,
                 shape =  strategy)) +
  geom_point(size = 4, alpha = 1, na.rm = T) +
  geom_vline(xintercept = 0, color = "black") +
  geom_hline(yintercept = 0, color = "black") +
  labs(x = "Incremental QALYs (thousands)", 
  ...
  theme_bw() +
  coord_cartesian(xlim = c(-40, 10), ylim = c(-5, 25)) +
