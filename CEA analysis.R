...
targetfunc <- function(dt) {
  dt <- subset(dt, ISO3 == "150+" & AGERP > 10 & AGERP < 35)
  dt
}
...
# total baseline QALYS ????????????????????NOT CORRECT
a <- which( colnames(base) == "v.p.sus" )
b <- which( colnames(base) == "v.p.emigrate" )
base$Vsum <- rowSums(base[, a:b], na.rm = TRUE)
a <- which( colnames(base) == "SQ.p.sus" )
b <- which( colnames(base) == "SQ.p.emigrate" )
base$SQsum <- rowSums(base[, a:b], na.rm =TRUE)
qalybase <- sum(base$Vsum) - sum(base$SQsum)

# total baseline tb cases ????????????????????NOT CORRECT
basetbcount <- base[YARP < YEAR+1, sum(p.tb)]
...
tabfunc<-function(dt) {
  ...
  cdt[, agediff:= AGEP - AGERP]
  cdt <- subset(cdt, agediff == 1)
  ...
  # total additional cost of strategy
...
dt[YARP == 2023, sum(NUMP), by = cycle]
