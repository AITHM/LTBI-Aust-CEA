...
repquantiles <- function(dt) {
  ...
  qt <- as.data.table(qt)
  head <- cbind(head, qt)
  # yob.split <- split(dt, dt$yob)
  # yob.split <- lapply(yob.split, function(dt) {
  #   g <- which(colnames(dt) == firstrep)
  #   h <- which(colnames(dt) == lastrep)
  #   headcols <- g - 1
  #   head <- dt[, 1:headcols]
  ...
  #   m <- as.matrix(m)
  #   set.seed(10000)
  #   probs <- rpert(5, min = 0.25, mode = 0.50, max = 0.75)
  #   probs <- round(probs, digits = 2)
  #   qt <- rowQuantiles(m, probs = probs, drop = FALSE)
  #   qt <- as.data.table(qt)
  #   head <- cbind(head, qt)
  # })
  # dt <- rbindlist(yob.split)
  dt
}
...
check <- check %>%
  group_by(YARP, ISO3, CNSY, SEXP, AGEP) %>% # aggregating by ysa 
...
#set directory to the folder where the file is to be saved
setwd("H:/Katie/PhD/CEA/Data/New R data for my model")
# Save the  object
